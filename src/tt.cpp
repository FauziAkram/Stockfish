...
std::tuple<bool, TTData, TTWriter> TranspositionTable::probe(const Key key) const {
    ...
    for (int i = 1; i < ClusterSize; ++i)
        if (replace->depth8 - replace->relative_age(generation8) * 2 + (replace->genBound8 & BOUND_EXACT)
        > tte[i].depth8 - tte[i].relative_age(generation8) * 2 + (tte[i].genBound8 & BOUND_EXACT))
            replace = &tte[i];
