...
void TTEntry::save(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8) {
    ...
    const bool keyMatch = (uint16_t(k) == key16); // Store key match result once

    // Overwrite less valuable entries
    if (b == BOUND_EXACT || !keyMatch || d - DEPTH_ENTRY_OFFSET + 2 * pv > depth8 - 4 || relative_age(generation8))
    {
        assert(d > DEPTH_ENTRY_OFFSET);
        assert(d < 256 + DEPTH_ENTRY_OFFSET);

        // Decide which move to write: the new move 'm', unless keys match,
        // 'm' is none, and the old entry was valid (to preserve its move).
        Move moveToWrite = m;
        if (keyMatch && !m && is_occupied())
            moveToWrite = move16;
