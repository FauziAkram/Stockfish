...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return 8000 * pcv + 7518 * micv + 10293 * (wnpcv + bnpcv) + 7315 * cntcv;
}
...
int singular1_correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    const Color us    = pos.side_to_move();
    const auto  m     = (ss - 1)->currentMove;
    const auto  pcv   = w.pawnCorrectionHistory[pawn_structure_index<Correction>(pos)][us];
    const auto  micv  = w.minorPieceCorrectionHistory[minor_piece_index(pos)][us];
    ...
    return 7202 * pcv + 7075 * micv + 8236 * (wnpcv + bnpcv) + 6636 * cntcv;
}

int singular2_correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    const Color us    = pos.side_to_move();
    const auto  m     = (ss - 1)->currentMove;
    const auto  pcv   = w.pawnCorrectionHistory[pawn_structure_index<Correction>(pos)][us];
    const auto  micv  = w.minorPieceCorrectionHistory[minor_piece_index(pos)][us];
    ...
    return 7739 * pcv + 7390 * micv + 8727 * (wnpcv + bnpcv) + 6023 * cntcv;
}

int lmr_correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    const Color us    = pos.side_to_move();
    const auto  m     = (ss - 1)->currentMove;
    const auto  pcv   = w.pawnCorrectionHistory[pawn_structure_index<Correction>(pos)][us];
    const auto  micv  = w.minorPieceCorrectionHistory[minor_piece_index(pos)][us];
    ...
    return 7412 * pcv + 6721 * micv + 9606 * (wnpcv + bnpcv) + 6856 * cntcv;
}
