...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1498 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 947);
  }
  ...
  int stat_bonus(Depth d) {
    return std::min((11 * d + 287) * d - 365 , 1526);
  }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(10) + int(prev) * prev / 15680;
              alpha = std::max(prev - delta,-VALUE_INFINITE);
              beta  = std::min(prev + delta, VALUE_INFINITE);

              // Adjust optimism based on root move's previousScore
              int opt = 118 * prev / (std::abs(prev) + 167);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-19 * int((ss-1)->staticEval + ss->staticEval), -1938, 1938);
    ...
    // return a fail low.
    if (eval < alpha - 380 - 251 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        ...
        &&  eval >= beta
        &&  eval < 28854) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 19 * depth - improvement / 13 + 239 + complexity / 26
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 169, 7) + depth / 3 + 4 - (complexity > 811);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  ...
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 186 + 207 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 13
                  && ss->staticEval + 107 + 136 * lmrDepth + history / 50 <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5425)
              extension = 1;
      }
      ...
      r -= ss->statScore / (12990 + 4262 * (depth > 7 && depth < 19));
