         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
          ...
          && !mainThread->stopOnPonderhit)
      {
          double fallingEval = (70 + 14 * (mainThread->bestPreviousAverageScore - bestValue)
                                    +  6 * (mainThread->iterValue[iterIdx] - bestValue)) / 620.0;
          fallingEval = std::clamp(fallingEval, 0.5, 1.5);

          // If the bestMove is stable over several iterations, reduce time accordingly
          timeReduction = lastBestMoveDepth + 8 < completedDepth ? 1.58 : 0.69;
          double reduction = (1.3 + mainThread->previousTimeReduction) / (2.22 * timeReduction);
