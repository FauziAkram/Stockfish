           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                int depthBonus = (rootDepth >= 6 && rootDepth <= 11) ? 1 : 0;
                int fhPenalty = (5 + 4 * (rootDepth > 15)) * failedHighCnt / 4;

                Depth adjustedDepth =
                  std::max(1, rootDepth + depthBonus - fhPenalty - (4 * searchAgainCounter + 4) / 5);
