...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonus = (118 * (depth > 5) + 38 * !allNode + 183 * ((ss - 1)->moveCount > 8)
                     + 114 * (!ss->inCheck && (bestValue <= ss->staticEval - 108 || bestValue < alpha - 64 * depth))
                     + 133 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 83));
