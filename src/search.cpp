           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        if (Limits.use_time_management() && !Threads.stop && !mainThread->stopOnPonderhit)
        {
            double fallingEval = (65 + 14 * (mainThread->bestPreviousAverageScore - bestValue)
                               ...
                               / 620.2;
            fallingEval = std::clamp(fallingEval, 0.51, 1.54);

            // If the bestMove is stable over several iterations, reduce time accordingly
            timeReduction    = lastBestMoveDepth + 8 < completedDepth ? 1.6 : 0.75;
            double reduction = (1.34 + mainThread->previousTimeReduction) / (2.33 * timeReduction);
            ...
            if (rootMoves.size() == 1)
                totalTime = std::min(512.0, totalTime);
