...
namespace Stockfish {
int xx1=120, xx2=5, xx3=120;
int xx4=1652, xx5=1546, xx6=18782;
TUNE(xx1);
TUNE(SetRange(-100, 100), xx2);
TUNE(xx3,xx4,xx5,xx6);
namespace Search {
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -xx4, xx5)
...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + ((ss - 1)->statScore < -xx6)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
