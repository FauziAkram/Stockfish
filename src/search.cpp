...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 120 - 27 * noTtCutNode;
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return 7027 * pcv + 6904 * micv + 7910 * (wnpcv + bnpcv) + 6088 * cntcv;
}
...
int stat_bonus(Depth d) { return std::min(167 * d - 97, 1536); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(851 * d - 233, 2678); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 5 + std::abs(rootMoves[pvIdx].meanSquaredScore) / 12620;
...
void Search::Worker::clear() {
    ...
    captureHistory.fill(-651);
    pawnHistory.fill(-1276);
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(2952 / 128.0 * std::log(i));
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (cutNode == (ttData.value >= beta) || depth > 9))
    {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move, stat_bonus(depth) * 707 / 1024);

            // Extra penalty for early quiet moves of the previous ply
            if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 3 && !priorCapture)
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                              -stat_malus(depth + 1) * 1023 / 1024);
        }
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1845, 1420) + 615;
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << bonus * 1180 / 1024;
    ...
    if (!PvNode && eval < alpha - 466 - 296 * depth * depth)
    ...
    if (!ss->ttPv && depth < 14
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving, opponentWorsening)
               - (ss - 1)->statScore / 314 + 37 - std::abs(correctionValue) / 132407
             >= beta
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - 20 * depth + 456 - 57 * improving && !excludedMove
        && pos.non_pawn_material(us) && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        assert(eval - beta >= 0);

        // Null move dynamic reduction based on depth and eval
        Depth R = std::min(int(eval - beta) / 243, 6) + 39 * depth / 128 + 5;
    ...
    improving |= ss->staticEval >= beta + 95;
    ...
    probCutBeta = beta + 190 - 57 * improving;
    if (depth >= 3
        && !is_decisive(beta)
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 225 + 239 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 36, -160 * depth, 130 * depth);
                if (!pos.see_ge(move, -148 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (history < -4176 * depth)
                    continue;

                history += 64 * thisThread->mainHistory[us][move.from_to()] / 32;

                lmrDepth += history / 3738;

                Value futilityValue = ss->staticEval + (bestMove ? 50 : 135) + 144 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttData.move && !excludedMove
                && depth >= 5 - (thisThread->completedDepth > 30) + ss->ttPv
                && is_valid(ttData.value) && !is_decisive(ttData.value)
                && (ttData.bound & BOUND_LOWER) && ttData.depth >= depth - 3)
            {
                Value singularBeta  = ttData.value - (57 + 79 * (ss->ttPv && !PvNode)) * depth / 56;
                ...
                {
                    int corrValAdj1  = std::abs(correctionValue) / 264266;
                    int corrValAdj2  = std::abs(correctionValue) / 252425;
                    int doubleMargin = 264 * PvNode - 189 * !ttCapture - corrValAdj1;
                    int tripleMargin =
                      103 + 283 * PvNode - 249 * !ttCapture + 99 * ss->ttPv - corrValAdj2;
        ...
        if (ss->ttPv)
            r -= 2172 + (ttData.value > alpha) * 875 + (ttData.depth >= depth) * 978;
        ...
        r -= std::abs(correctionValue) / 30668;

        // Increase reduction for cut nodes
        if (cutNode)
            r += 2744 - (ttData.depth >= depth && ss->ttPv) * 1111;
        ...
        if (capture)
            ss->statScore =
              7127 * int(PieceValue[pos.captured_piece()]) / 1024
              + thisThread->captureHistory[movedPiece][move.to_sq()][type_of(pos.captured_piece())]
              - 4444;
        else
            ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
                          + (*contHist[1])[movedPiece][move.to_sq()] - 3615;

        // Decrease/increase reduction for moves with a good/bad history
        r -= ss->statScore * 1429 / 16384;
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 3589) - (r > 5085 && newDepth > 3), !cutNode);
        }
    ...
    {
        int bonusScale = (120 * (depth > 5) + 35 * !allNode + 157 * ((ss - 1)->moveCount > 8)
                          + 133 * (!ss->inCheck && bestValue <= ss->staticEval - 110)
                          + 121 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 84)
                          + 84 * ((ss - 1)->isTTMove) + std::min(-(ss - 1)->statScore / 105, 297));
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 318;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * 690 / rootDelta + !i * reductionScale * 386 / 1024 + 1044;
}
...
void update_all_stats(const Position&      pos,
    ...
    int bonus = stat_bonus(depth) + 303 * isTTMove;
    int malus = stat_malus(depth) - 31 * (moveCount - 1);

    if (!pos.capture_stage(bestMove))
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * 1185 / 1024);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * 1133 / 1024);
    }
    else
    {
    ...
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -malus * 970 / 1024);
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[moved_piece][move.to_sq()][captured] << -malus * 1257 / 1024;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    static constexpr std::array<ConthistBonus, 6> conthist_bonuses = {
      {{1, 990}, {2, 661}, {3, 327}, {4, 535}, {5, 123}, {6, 518}}};
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.from_to()] << bonus * 862 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(), bonus * 944 / 1024);

    int pIndex = pawn_structure_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()] << bonus * 608 / 1024;
}
