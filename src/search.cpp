...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1450 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 936);
  }
  ...
  int stat_bonus(Depth d) {
    return std::min((11 * d + 284) * d - 366 , 1640);
  }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(14) + int(prev) * prev / 15500;
              alpha = std::max(prev - delta,-VALUE_INFINITE);
              beta  = std::min(prev + delta, VALUE_INFINITE);

              // Adjust optimism based on root move's previousScore
              int opt = 116 * prev / (std::abs(prev) + 172);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // return a fail low.
    if (eval < alpha - 394 - 256 * depth * depth)
    {
    ...
    if (   !PvNode
        ...
        && (ss-1)->statScore < 19200
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 19 * depth - improvement / 13 + 235 + complexity / 32
        && !excludedMove
    ...
    probCutBeta = beta + 180 - 53 * improving;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 13
                  && ss->staticEval + 109 + 128 * lmrDepth + history / 64 <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5650)
              extension = 1;
      }
      ...
      r -= ss->statScore / (12600 + 4800 * (depth > 7 && depth < 19));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (alpha + 72 + 10 * (newDepth - d));
