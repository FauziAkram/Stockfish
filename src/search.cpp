...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (auto& prevMove = (ss - 1)->currentMove; 
    prevMove.is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
    int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1831, 1428) + 623;
    thisThread->mainHistory[~us][prevMove.from_to()] << bonus;

    auto prevPiece = pos.piece_on(prevSq);
    if (type_of(prevPiece) != PAWN && prevMove.type_of() != PROMOTION)
        thisThread->pawnHistory[pawn_structure_index(pos)][prevPiece][prevSq] << bonus;
    }
