...
namespace Stockfish {
int xx1=200, xx2=100, xx3=100, xx4=100, xx5=14767, xx0=381700;
TUNE(SetRange(0, 500), xx1,xx2,xx3,xx4);
TUNE(xx5,xx0);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 1749600
...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + ((ss - 1)->statScore < -1878200)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
