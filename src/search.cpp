...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1440 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 948);
  }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(11) + int(prev) * prev / 14444;
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // return a fail low.
    if (eval < alpha - 420 - 255 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        &&  depth < 8
        &&  eval - futility_margin(depth, improving) - (ss-1)->statScore / 286 >= beta
        &&  eval >= beta
        &&  eval < 29520) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 165, 6) + depth / 3 + 4 - (complexity > 835);
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  ...
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 200 + 200 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
      ...
      r -= ss->statScore / (12800 + 4480 * (depth > 7 && depth < 19));
