...
constexpr int futility_move_count(bool improving, Depth depth, int staticEval) {
    return improving ? (3 + depth * depth) : (3 + depth * depth) / 2 + std::clamp(staticEval / 150, -2, 2);
}
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            moveCountPruning = moveCount >= futility_move_count(improving, depth, !ss->inCheck ? ss->staticEval : -16);
