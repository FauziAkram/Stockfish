...
void update_correction_history(const Position& pos,
    ...
    workerThread.pawnCorrectionHistory[pawn_structure_index<Correction>(pos)][us]
      << bonus * xx8 / 128;
    workerThread.minorPieceCorrectionHistory[minor_piece_index(pos)][us] << bonus * xx9 / 128;
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 5 + std::abs(rootMoves[pvIdx].meanSquaredScore) / xx15;
...
void Search::Worker::clear() {
    mainHistory.fill(xx18);
    lowPlyHistory.fill(xx19);
    captureHistory.fill(-xx20);
    pawnHistory.fill(-xx21);
    pawnCorrectionHistory.fill(xx22);
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(xx25 / 128.0 * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -xx30, xx31) + xx32;
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << bonus * xx33 / 1024;
    ...
    if (priorReduction >= 3 && !opponentWorsening)
        depth++;
    if (priorReduction >= 1 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > xx35)
        depth--;

    // Step 7. Razoring
    // If eval is really low, skip search entirely and return the qsearch value.
    // For PvNodes, we must have a guard against mates being returned.
    if (!PvNode && eval < alpha - xx36 - xx37 * depth * depth)
    ...
    if (!ss->ttPv && depth < 14
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving, opponentWorsening)
               - (ss - 1)->statScore / xx38 + xx39 + ((eval - beta) / 8)
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - xx41 * depth + xx42 && !excludedMove && pos.non_pawn_material(us)
        && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        assert(eval - beta >= 0);

        // Null move dynamic reduction based on depth and eval
        Depth R = std::min(int(eval - beta) / xx43, 6) + depth / 3 + 5;
    ...
    improving |= ss->staticEval >= beta + xx44;

    // Step 10. Internal iterative reductions
    // For PV nodes without a ttMove as well as for deep enough cutNodes, we decrease depth.
    // (* Scaler) Especially if they make IIR more aggressive.
    if (((PvNode || cutNode) && depth >= (PvNode? xx45: xx46)) && !ttData.move)
        depth--;

    // Step 11. ProbCut
    // If we have a good enough capture (or queen promotion) and a reduced search
    // returns a value much above beta, we can (almost) safely prune the previous move.
    probCutBeta = beta + xx47 - xx48 * improving;
    if (depth >= 3
        && !is_decisive(beta)
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + xx52 + xx53 * lmrDepth
                                        + PieceValue[capturedPiece] + xx54 * captHist / 1024;
                    if (futilityValue <= alpha)
                        continue;
                }

                // SEE based pruning for captures and checks
                int seeHist = std::clamp(captHist / xx55, -xx56 * depth, xx57 * depth);
                if (!pos.see_ge(move, -xx58 * depth - seeHist))
                {
                    bool skip = true;
                    if (depth > xx168 && !capture && givesCheck && alpha < 0
            ...
            else
            {
                ...
                if (history < -xx59 * depth)
                    continue;

                history += xx60 * thisThread->mainHistory[us][move.from_to()] / 32;

                lmrDepth += history / xx61;

                Value futilityValue = ss->staticEval + (bestMove ? xx62 : xx63) + xx64 * lmrDepth
                                    + xx65 * (bestValue < ss->staticEval - xx66 && ss->staticEval > alpha - xx67);
                ...
                if (!pos.see_ge(move, -xx68 * lmrDepth * lmrDepth))
                    continue;
            }
        }
        ...
        // We take care to overdo so search gets stuck.
        {
            ...
            {
                Value singularBeta  = ttData.value - (xx70 + xx71 * (ss->ttPv && !PvNode)) * depth / xx72;
                ...
                {
                    int corrValAdj1  = std::abs(correctionValue) / xx73;
                    int corrValAdj2  = std::abs(correctionValue) / xx74;
                    int doubleMargin = xx75 + xx76 * PvNode - xx77 * !ttCapture - corrValAdj1
                                     - xx78 * ttMoveHistory / 131072
                                     - (ss->ply * 2 > thisThread->rootDepth * 3) * xx79;
                    int tripleMargin = xx80 + xx81 * PvNode - xx82 * !ttCapture + xx83 * ss->ttPv
                                     - corrValAdj2 - (ss->ply * 2 > thisThread->rootDepth * 3) * xx84;
        ...
        if (ss->ttPv)
            r -= xx85 + PvNode * xx86 + (ttData.value > alpha) * xx87
               + (ttData.depth >= depth) * (xx88 + cutNode * xx89);

        // These reduction adjustments have no proven non-linear scaling

        r += xx90 - moveCount * xx91;

        r -= std::abs(correctionValue) / xx92;

        if (PvNode && std::abs(bestValue) <= xx93)
            r -= risk_tolerance(bestValue);

        // Increase reduction for cut nodes
        if (cutNode)
            r += xx94 + xx95 * !ttData.move;
        ...
        if (capture)
            ss->statScore =
              xx101 * int(PieceValue[pos.captured_piece()]) / 128
              + thisThread->captureHistory[movedPiece][move.to_sq()][type_of(pos.captured_piece())]
              - xx102;
        else if (ss->inCheck)
            ss->statScore = thisThread->mainHistory[us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()] - xx103;
        else
            ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
                          + (*contHist[1])[movedPiece][move.to_sq()] - xx104;

        // Decrease/increase reduction for moves with a good/bad history
        r -= ss->statScore * xx105 / 16384;
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + xx106 + 2 * newDepth);
                const bool doShallowerSearch = value < bestValue + xx107;
                ...
                update_continuation_histories(ss, movedPiece, move.to_sq(), xx108);
            }
            else if (value > alpha && value < bestValue + xx109)
            {
                newDepth--;
                if (value < bestValue + xx110)
                    newDepth--;
            }
        }
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            r -= xx112 * ttMoveHistory / 8192;

            // Note that if expected reduction is high, we reduce search depth here
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > xx113) - (r > xx114 && newDepth > 2), !cutNode);
        }
    ...
    else if (bestMove)
    {
        ...
        if (!PvNode)
        {
            int bonus = (ttData.move == move) ? xx115 : -xx116;
            ttMoveHistory << bonus;
        }
    }
    ...
    {
        int bonusScale =
          (std::min(xx117 * depth - xx118, xx119) + xx120 * !allNode + xx121 * ((ss - 1)->moveCount > 8)
           + xx122 * (!ss->inCheck && bestValue <= ss->staticEval - xx123)
           + xx124 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - xx125)
           + xx126 * ((ss - 1)->isTTMove) + xx127 * (ss->cutoffCnt <= 3)
           + std::min(-(ss - 1)->statScore / xx128, xx129));

        bonusScale = std::max(bonusScale, 0);

        const int scaledBonus = std::min(xx130 * depth - xx131, xx132) * bonusScale;

        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      scaledBonus * xx133 / 32768);

        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << scaledBonus * xx134 / 32768;

        if (type_of(pos.piece_on(prevSq)) != PAWN && ((ss - 1)->currentMove).type_of() != PROMOTION)
            thisThread->pawnHistory[pawn_structure_index(pos)][pos.piece_on(prevSq)][prevSq]
              << scaledBonus * xx135 / 32768;
    }
    ...
    else if (priorCapture && prevSq != SQ_NONE)
    {
        ...
        thisThread->captureHistory[pos.piece_on(prevSq)][prevSq][type_of(capturedPiece)] << xx136;
    }
    ...
    if (!excludedMove && !(rootNode && thisThread->pvIdx))
        ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), ss->ttPv,
                       ...
                       : PvNode && bestMove ? BOUND_EXACT
                                            : BOUND_UPPER,
                       moveCount != 0 ? depth : std::min(MAX_PLY - 1, depth + xx137), bestMove,
                       unadjustedStaticEval, tt.generation());
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + xx138;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * xx141 / rootDelta + !i * reductionScale * xx142 / 512 + xx143;
}
...
void update_all_stats(const Position&      pos,
    ...
    int bonus = std::min(xx144 * depth - xx145, xx146) + xx147 * isTTMove;
    int malus = std::min(xx148 * depth - xx149, xx150) - xx151 * (moveCount - 1);

    if (!pos.capture_stage(bestMove))
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * xx152 / 1024);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * xx153 / 1024);
    }
    else
    {
        // Increase stats for the best move in case it was a capture move
        captured = type_of(pos.piece_on(bestMove.to_sq()));
        captureHistory[moved_piece][bestMove.to_sq()][captured] << bonus * xx154 / 1024;
    }

    // Extra penalty for a quiet early move that was not a TT move in
    // previous ply when it gets refuted.
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -malus * xx155 / 1024);
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[moved_piece][move.to_sq()][captured] << -malus * xx156 / 1024;
    }
}
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.from_to()] << bonus * xx163 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(),
                                  bonus * (bonus > 0 ? xx164 : xx165) / 1024);

    int pIndex = pawn_structure_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()]
      << bonus * (bonus > 0 ? xx166 : xx167) / 1024;
}
