...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // return a fail low.
    if (eval < alpha - 426 - 262 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        &&  depth < 9
        &&  eval >= beta
        &&  eval < 24176) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        {
        if (eval - futility_margin(depth, improving) - (ss-1)->statScore / 267 >= beta)
            return eval;
        else if (eval - futility_margin(depth, improving) - (ss-1)->statScore / 267 >= beta - 20)
