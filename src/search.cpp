...
constexpr int futility_move_count(bool improving, bool worsening, Depth depth) {
    return improving ? (3 + depth * depth) : worsening ? (2 * (3 + depth * depth) / 7) : (3 + depth * depth) / 2;
}
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool     givesCheck, improving, worsening, priorCapture;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        Depth r = reduction(worsening, depth, moveCount, delta);
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && worsening && lmrDepth < 7 && !ss->inCheck)
                {
