...
namespace Stockfish {
int xx1=400, xx2=413, xx3=4000, xx4=7011;
        ...
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (ttCapture)
                {
                   if (thisThread->captureHistory[pos.moved_piece(ttMove)][to_sq(ttMove)][type_of(pos.piece_on(to_sq(ttMove)))] < -xx1)
