...
namespace Stockfish {
int xx1=186, xx2=285, xx3=1524, xx4=707, xx5=260, xx6=2073, yy1=0, yy2=0;
TUNE(xx1,xx2,xx3,xx4,xx5,xx6);
TUNE(SetRange(-40, 40), yy1,yy2);
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        ...
        if (ttMove && ttValue >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_stats(pos, ss, *this, ttMove, stat_bonus(pos, depth));
...
moves_loop:  // When in check, search starts here
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      stat_bonus(pos, depth) * bonus / 100);
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << stat_bonus(pos, depth) * bonus / 200;


        if (type_of(pos.piece_on(prevSq)) != PAWN && ((ss - 1)->currentMove).type_of() != PROMOTION)
            thisThread->pawnHistory[pawn_structure_index(pos)][pos.piece_on(prevSq)][prevSq]
              << stat_bonus(pos, depth) * bonus / 25;
    }
...
void update_all_stats(const Position& pos,
    ...
    int quietMoveMalus = stat_malus(pos, depth);
