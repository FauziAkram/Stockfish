...
namespace Stockfish {
int xx1=200, xx2=17662, xx3=16, xx4=105;
TUNE(SetRange(-200, 600), xx1);
TUNE(xx2,xx3,xx4);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (std::abs(ttValue) < VALUE_TB_WIN_IN_MAX_PLY && ss->staticEval != VALUE_NONE)
          ss->statScore += (std::abs(ttValue - ss->staticEval) * xx1) / 100;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (xx2 - std::min(depth, xx3) * xx4);
