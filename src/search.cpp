  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!ss->ttPv && depth < 14
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving, opponentWorsening)
               - (ss - 1)->statScore / 301 + 37 - std::abs(futility_correction_value(*thisThread, pos, ss)) / 139878
             >= beta
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && (ttData.bound & BOUND_LOWER) && ttData.depth >= depth - 3)
            {
                ...
                {
                    int corrValAdj1  = std::abs(singular1_correction_value(*thisThread, pos, ss)) / 248873;
                int corrValAdj2  = std::abs(singular2_correction_value(*thisThread, pos, ss)) / 255331;
        ...
        r -= std::abs(lmr_correction_value(*thisThread, pos, ss)) / 29696;
