...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1385 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 937);
  }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(13) + int(prev) * prev / 15400;
              alpha = std::max(prev - delta,-VALUE_INFINITE);
              beta  = std::min(prev + delta, VALUE_INFINITE);

              // Adjust optimism based on root move's previousScore
              int opt = 116 * prev / (std::abs(prev) + 174);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-20 * int((ss-1)->staticEval + ss->staticEval), -1831, 1831);
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 20 * depth - improvement / 14 + 235 + complexity / 27
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 165, 5) + depth / 3 + 4 - (complexity > 800);
    ...
    probCutBeta = beta + 180 - 52 * improving;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  ...
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 185 + 215 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
      ...
      r -= ss->statScore / (12800 + 4737 * (depth > 7 && depth < 19));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doEvenDeeperSearch = value > alpha + 54 && ss->doubleExtensions <= 5;
