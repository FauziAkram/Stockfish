...
int stat_malus(Depth d) { return std::min(519 * d - 306, 1220); }
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    probCutBeta = beta + 170 - 68 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        Depth r = reduction(improving, depth, moveCount, delta, cutNode);
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta, bool cutNode) {
    int reductionScale = reductions[cutNode][d] * reductions[cutNode][mn];
