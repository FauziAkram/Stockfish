...
namespace Stockfish {
int yy1=0, yy2=0, xx1=1, xx2=1, xx3=1, xx4=1, xx5=1, xx6=1, xx7=1, xx8=-1, xx9 =0,	xx10 =0,	xx11 =0,	xx12 =0,	xx13 =0,	xx14 =0,	xx15 =0,	xx16 =0,
...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonus = xx1 * (depth > 4) + xx2 * (depth > 5) + xx3 * (PvNode || cutNode) + xx4 * ((ss - 1)->statScore < -13241)
                  + xx5 * ((ss - 1)->moveCount > 10) + xx6 * (!ss->inCheck && bestValue <= ss->staticEval - 127)
                  + xx7 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 74)
                  + xx8 * (bestValue >= ss->staticEval + 1200)
+ xx9 * (PvNode)
+ xx10 * (ss->ttPv)
+ xx11 * (cutNode)
+ xx12 * (improving)
...
+ xx15 * (capture)
+ xx16 * (givesCheck)
+ xx17 * (((ss + 1)->cutoffCnt > 3))
+ xx18 * (type_of(movedPiece) == PAWN)
+ xx19 * (type_of(movedPiece) == KING)
+ xx20 * ((ss - 1)->currentMove == Move::null())
+ xx21 * (move == ttMove)
+ xx22 * (move == ss->killers[0])
+ xx23 * (move == ss->killers[1])
+ xx24 * (move == countermove)
+ xx25 * (extension > 0)
+ xx26 * (extension < 0)
+ xx27 * (extension > 2)
+ xx28 * (extension < 2)
...
+ xx34 * ((ss - 1)->ttPv)
+ xx35 * ((ss - 1)->ttHit)
+ xx36 * (bool((ss - 1)->excludedMove))
+ xx37 * (bool(excludedMove))
+ xx38 * (ss->inCheck)
+ xx39 * (ttValue < ss->staticEval)
+ xx40 * (alpha < ss->staticEval)
+ xx41 * (opponentWorsening)
+ xx42 * (depth > 8)
+ xx43 * (depth > 12)
+ xx44 * (depth > 15)
+ xx45 * (depth < 5)
