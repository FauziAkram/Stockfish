...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture && !(ss-2)->inCheck)
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval) - int((ss - 2)->staticEval), -1881, 1413) + 616;
