...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 124 - 44 * noTtCutNode;
    Value improvingDeduction = 52 * improving * futilityMult / 32;
    Value worseningDeduction = (316 + 49 * improving) * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 9020;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(214 * d - 304, 16, 1304); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 572 * d - 292 : 1355); }
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-14 * int((ss - 1)->staticEval + ss->staticEval), -1644, 1475);
    ...
    if (eval < alpha - 471 - (272 - 148 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 18001
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 316 && !excludedMove
    ...
    probCutBeta = beta + 169 - 64 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 272 + 274 * lmrDepth
            ...
            else
            {
                ...
                lmrDepth += history / 4952;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 54 ? 129 : 60) + 131 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (63 + 59 * (ss->ttPv && !PvNode)) * depth / 63;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 11 && beta < 13546 && value > -13652)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -14400)
                  + ((ss - 1)->moveCount > 11)
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1230 - delta * 829 / rootDelta) / 1024 + (!i && reductionScale > 1025);
}
