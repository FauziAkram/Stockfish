...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (priorReduction >= 3 && !opponentWorsening)
        depth++;
    if (priorReduction >= 2 && depth >= (2 + ((ss->staticEval + (ss - 1)->staticEval > 240) && (priorReduction >= 3))) && ss->staticEval + (ss - 1)->staticEval > 173)
        depth -= 1 + ((ss->staticEval + (ss - 1)->staticEval > 240) && (priorReduction >= 3));
