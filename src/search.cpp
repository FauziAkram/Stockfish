...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -1561, 1408);
    ...
    if (PvNode && ttMove && (tte->bound() & BOUND_UPPER) && ttValue <= alpha
        && !pos.capture(ttMove) && (*contHist[0])[pos.moved_piece(ttMove)][to_sq(ttMove)] < -14982)
        depth -= 2;
    ...
    probCutBeta = beta + 154 - 69 * improving;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 13145 && value > -11665)
                        depth -= 2;
