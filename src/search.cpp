...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonus = (117 * (depth > 5) + 39 * !allNode + 182 * ((ss - 1)->moveCount > 8)
                     + 115 * (!ss->inCheck && (bestValue <= ss->staticEval - 108 || bestValue < alpha))
                     + 132 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 83));
