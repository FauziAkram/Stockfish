...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 157200
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        ss->statScore = 21 * thisThread->mainHistory[us][move.from_to()]
                      + 13 * (*contHist[0])[movedPiece][move.to_sq()]
                      + 10 * (*contHist[1])[movedPiece][move.to_sq()]
                      +  8 * (*contHist[3])[movedPiece][move.to_sq()] - 41181;

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / 167254;
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + ((ss - 1)->statScore < -180878)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
