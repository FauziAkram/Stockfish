...
namespace {
  ...
  int stat_bonus(Depth d) {
    return std::min((12 * d + 282) * d - 349 , 1556);
  }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          r -= ss->statScore / (12852 + 4266 * (depth > 7 && depth < 19));
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (alpha + 56 + 11 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 628;
