...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1647 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 929);
  }
  ...
  int stat_bonus(Depth d) {
    return std::min((12 * d + 284) * d - 346 , 1480);
  }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(10) + int(prev) * prev / 15688;
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-19 * int((ss-1)->staticEval + ss->staticEval), -1930, 1930);
    ...
    // return a fail low.
    if (eval < alpha - 371 - 252 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        ...
        &&  eval >= beta
        &&  eval < 28356) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 17028
        &&  eval >= beta
        &&  eval >= ss->staticEval
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 174, 7) + depth / 3 + 4 - (complexity > 874);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 13
                  && ss->staticEval + 106 + 145 * lmrDepth + history / 53 <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5176)
              extension = 1;
      }
      ...
      r -= ss->statScore / (13154 + 4222 * (depth > 7 && depth < 19));
