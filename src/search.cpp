...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1658 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 924);
  }
  ...
  int stat_bonus(Depth d) {
    return std::min((12 * d + 278) * d - 343 , 1476);
  }
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(10) + int(prev) * prev / 15870;
              alpha = std::max(prev - delta,-VALUE_INFINITE);
              beta  = std::min(prev + delta, VALUE_INFINITE);

              // Adjust optimism based on root move's previousScore
              int opt = 118 * prev / (std::abs(prev) + 165);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-19 * int((ss-1)->staticEval + ss->staticEval), -1952, 1952);
    ...
    // return a fail low.
    if (eval < alpha - 372 - 251 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        ...
        &&  eval >= beta
        &&  eval < 27868) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 17092
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 20 * depth - improvement / 13 + 234 + complexity / 25
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 173, 7) + depth / 3 + 4 - (complexity > 850);
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  ...
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 180 + 199 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 13
                  && ss->staticEval + 106 + 144 * lmrDepth + history / 52 <= alpha)
                  continue;
      ...
      r -= ss->statScore / (13124 + 4282 * (depth > 7 && depth < 19));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doEvenDeeperSearch = value > alpha + 596 && ss->doubleExtensions <= 5;
    ...
    */
    ...
    else if (   (depth >= 5 || PvNode || bestValue < alpha - 61 * depth)
             && !priorCapture)
    {
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    {
        int bonus2 = bestValue > beta + 139 ? bonus1               // larger bonus
