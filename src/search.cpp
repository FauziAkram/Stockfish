...
namespace Stockfish {
int xx1=1024, xx2=1024, xx3=10, xx4=10, xx5=10;
TUNE(xx1,xx2);
TUNE(SetRange(0, 40), xx3,xx4,xx5);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if ((ss + 1)->cutoffCnt > 0)
            r += xx1 * ((ss + 1)->cutoffCnt > 2) + xx2 * allNode + (((10 * MAX_PLY - xx3 * depth) * (xx4 * allNode) * (xx5 * (ss + 1)->cutoffCnt)) / 1000);
