...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        ...
        Depth R = std::min(int(eval - beta) / 243, 6) + 91 * depth / 300 + 5;
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    Value futilityValue = ss->staticEval + 225 + 239 * lmrDepth
                                        + PieceValue[capturedPiece] + 91 * captHist / 700;
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value =
              -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                             newDepth - (r > xx129) - (r > 5085 && newDepth > 3), !cutNode);
        }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * 690 / rootDelta + !i * reductionScale * 193 / 512 + 1044;
}
