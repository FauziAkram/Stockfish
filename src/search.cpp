  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (   !givesCheck
                &&  to_sq(move) != prevSq
                &&  futilityBase + 31 > -VALUE_KNOWN_WIN
                &&  type_of(move) != PROMOTION)
            {
                if (moveCount > 2)
                    continue;

                futilityValue = futilityBase + 32 + PieceValue[pos.piece_on(to_sq(move))];
                ...
                if (futilityBase + 45 <= alpha && !pos.see_ge(move, VALUE_ZERO + 1))
                {
                    bestValue = std::max(bestValue, futilityBase - 42);
                    continue;
                }

                // If static exchange evaluation is much worse than what is needed to not
                // fall below alpha we can prune this move
                if (futilityBase > alpha && !pos.see_ge(move, (alpha - futilityBase + 24) * 4))
                {
                    bestValue = alpha;
                    continue;
                }
