...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // return a fail low.
    if (eval < alpha - 426 - 263 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        ...
        {
        if (eval - futility_margin(depth, improving) - (ss-1)->statScore / 271 >= beta)
            return eval;
        else if (eval - futility_margin(depth, improving) - (ss-1)->statScore / 271 >= beta - 20)
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 21 * depth - improvement / 13 + 253 + complexity / 25
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 173, 6) + depth / 3 + 4 - (complexity > 825);
