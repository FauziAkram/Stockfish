...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if ((ss + 1)->cutoffCnt > 955)
            r += 245 + 914 * ((ss + 1)->cutoffCnt > 2138) + 1136 * allNode;

        // For first picked move (ttMove) reduce reduction
        else if (move == ttData.move)
            r = std::max(-64, r - 2636 + 165 * cutNode);
        ...
        if (value + inc > bestValue)
        {
            ...
            if (value + inc > alpha)
            {
                ...
                if (value >= beta)
                {
                    ...
                    ss->cutoffCnt += 951 * ((extension < 2) || PvNode) - 174 * (extension < 2) + 94 * PvNode;
