...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (priorReduction >= 2921 && !opponentWorsening)
        depth++;
    if (priorReduction >= 2156 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 167)
        depth--;
    ...
    if (!allNode && depth >= 6 && !ttData.move && priorReduction <= 3227)
        depth--;
