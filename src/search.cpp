...
namespace Stockfish {
int xx1=51, xx2=100, xx3=100, xx4=100;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
            ...
            && (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1)))
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch     = value > (bestValue + xx1 + (xx2 * r / 10));
                const bool doShallowerSearch  = value < (bestValue + (xx3 * d / 100) + (xx4 * r / 100));
