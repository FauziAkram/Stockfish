...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 116 - 27 * noTtCutNode;
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return 7011 * pcv + 6749 * micv + 7832 * (wnpcv + bnpcv) + 6069 * cntcv;
}
...
void update_correction_history(const Position& pos,
    ...
    static constexpr int nonPawnWeight = 167;
...
int stat_bonus(Depth d) { return std::min(162 * d - 97, 1579); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(827 * d - 238, 2764); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 5 + std::abs(rootMoves[pvIdx].meanSquaredScore) / 12810;
            Value avg = rootMoves[pvIdx].averageScore;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore
            optimism[us]  = 139 * avg / (std::abs(avg) + 82);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(2945 / 128.0 * std::log(i));
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (cutNode == (ttData.value >= beta) || depth > 9))
    {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move, stat_bonus(depth) * 745 / 1024);

            // Extra penalty for early quiet moves of the previous ply
            if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 3 && !priorCapture)
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                              -stat_malus(depth + 1) * 1021 / 1024);
        }
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1874, 1434) + 625;
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << bonus * 1159 / 1024;
    ...
    if (!PvNode && eval < alpha - 456 - 299 * depth * depth)
    ...
    if (!ss->ttPv && depth < 14
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving, opponentWorsening)
               - (ss - 1)->statScore / 320 + 37 - std::abs(correctionValue) / 132614
             >= beta
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - 21 * depth + 456 - 58 * improving && !excludedMove
        && pos.non_pawn_material(us) && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        assert(eval - beta >= 0);

        // Null move dynamic reduction based on depth and eval
        Depth R = std::min(int(eval - beta) / 240, 6) + 40 * depth / 128 + 5;
    ...
    improving |= ss->staticEval >= beta + 96;
    ...
    probCutBeta = beta + 189 - 56 * improving;
    if (depth >= 3
        && !is_decisive(beta)
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 233 + 239 * lmrDepth
                                        + PieceValue[capturedPiece] + 136 * captHist / 1024;
                    if (futilityValue <= alpha)
                        continue;
                }

                // SEE based pruning for captures and checks
                int seeHist = std::clamp(captHist / 36, -157 * depth, 132 * depth);
            ...
            else
            {
                ...
                if (history < -4142 * depth)
                    continue;

                history += 64 * thisThread->mainHistory[us][move.from_to()] / 32;

                lmrDepth += history / 3657;

                Value futilityValue = ss->staticEval + (bestMove ? 50 : 135) + 147 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttData.move && !excludedMove
                && depth >= 5 - (thisThread->completedDepth > 31) + ss->ttPv
                && is_valid(ttData.value) && !is_decisive(ttData.value)
                && (ttData.bound & BOUND_LOWER) && ttData.depth >= depth - 3)
            {
                Value singularBeta  = ttData.value - (56 + 80 * (ss->ttPv && !PvNode)) * depth / 57;
                ...
                {
                    int corrValAdj1  = std::abs(correctionValue) / 264674;
                    int corrValAdj2  = std::abs(correctionValue) / 253052;
                    int doubleMargin = 265 * PvNode - 185 * !ttCapture - corrValAdj1;
                    int tripleMargin =
                      100 + 283 * PvNode - 249 * !ttCapture + 101 * ss->ttPv - corrValAdj2;
        ...
        if (ss->ttPv)
            r -= 2201 + (ttData.value > alpha) * 900 + (ttData.depth >= depth) * 975;
        ...
        r -= std::abs(correctionValue) / 31118;

        // Increase reduction for cut nodes
        if (cutNode)
            r += 2676 - (ttData.depth >= depth && ss->ttPv) * 1135;
        ...
        if (capture)
            ...
            ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
                          + (*contHist[1])[movedPiece][move.to_sq()] - 3603;

        // Decrease/increase reduction for moves with a good/bad history
        r -= ss->statScore * 1418 / 16384;
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 3572) - (r > 5226 && newDepth > 3), !cutNode);
        }
    ...
    {
        int bonusScale = (119 * (depth > 5) + 35 * !allNode + 159 * ((ss - 1)->moveCount > 8)
                          + 133 * (!ss->inCheck && bestValue <= ss->staticEval - 109)
                          + 121 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 84)
                          + 83 * ((ss - 1)->isTTMove) + std::min(-(ss - 1)->statScore / 106, 308));
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 321;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * 712 / rootDelta + !i * reductionScale * 384 / 1024 + 1088;
}
...
void update_all_stats(const Position&      pos,
    ...
    int bonus = stat_bonus(depth) + 301 * isTTMove;
    int malus = stat_malus(depth) - 32 * (moveCount - 1);

    if (!pos.capture_stage(bestMove))
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * 1193 / 1024);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * 1142 / 1024);
    }
    else
    {
        // Increase stats for the best move in case it was a capture move
        captured = type_of(pos.piece_on(bestMove.to_sq()));
        captureHistory[moved_piece][bestMove.to_sq()][captured] << bonus * 1274 / 1024;
    }

    // Extra penalty for a quiet early move that was not a TT move in
    // previous ply when it gets refuted.
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -malus * 973 / 1024);
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[moved_piece][move.to_sq()][captured] << -malus * 1241 / 1024;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    static constexpr std::array<ConthistBonus, 6> conthist_bonuses = {
      {{1, 1009}, {2, 659}, {3, 327}, {4, 535}, {5, 122}, {6, 527}}};
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.from_to()] << bonus * 853 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(), bonus * 954 / 1024);
