...
namespace {
  ...
  Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((140 - 39 * noTtCutNode) * (d - improving));
  }
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        &&  depth < 9
        &&  eval - futility_margin(depth, cutNode && !ss->ttHit, improving) - (ss-1)->statScore / 284 >= beta
        &&  eval >= beta
        &&  eval < 26550) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;
...
moves_loop: // When in check, search starts here
    ...
    Value futMargin = Value(400 * depth);
    if (depth < 6 && ss->inCheck && !ss->ttPv && (tte->bound() & BOUND_LOWER) && ttValue >= beta + futMargin && ttValue < 25195)
        return ttValue;
