...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return 4143 * pcv + 3951 * micv + 5129 * (wnpcv + bnpcv) + 3437 * cntcv;
}
...
void update_correction_history(const Position& pos,
    ...
    static constexpr int nonPawnWeight = 174;
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 5 + std::abs(rootMoves[pvIdx].meanSquaredScore) / 11176;
            Value avg = rootMoves[pvIdx].averageScore;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore
            optimism[us]  = 143 * avg / (std::abs(avg) + 95);
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            if (rootMoves.size() == 1)
                totalTime = std::min(525.0, totalTime);
...
void Search::Worker::clear() {
    ...
    captureHistory.fill(-714);
    pawnHistory.fill(-1311);
    pawnCorrectionHistory.fill(12);
    minorPieceCorrectionHistory.fill(0);
    nonPawnCorrectionHistory.fill(-4);
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(2670 / 128.0 * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move,
                                       std::min(128 * depth - 79, 1186));

            // Extra penalty for early quiet moves of the previous ply
            if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 3 && !priorCapture)
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -2297);
        }
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1914, 1525) + 613;
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << bonus * 1144 / 1024;
    ...
    if (priorReduction >= 3 && !opponentWorsening)
        depth++;
    if (priorReduction >= 1 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 172)
        depth--;

    // Step 7. Razoring
    // If eval is really low, skip search entirely and return the qsearch value.
    // For PvNodes, we must have a guard against mates being returned.
    if (!PvNode && eval < alpha - 464 - 315 * depth * depth)
    ...
    // The depth condition is important for mate finding.
    {
        auto futility_margin = [&](Depth d) {
            Value futilityMult = 101 - 20 * (cutNode && !ss->ttHit);

            return futilityMult * d                      //
                 - improving * futilityMult * 2          //
                 - opponentWorsening * futilityMult / 3  //
                 + (ss - 1)->statScore / 377             //
                 + std::abs(correctionValue) / 90495;
        };
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - 18 * depth + 398 && !excludedMove && pos.non_pawn_material(us)
    ...
    probCutBeta = beta + 210 - 57 * improving;
    if (depth >= 3
        && !is_decisive(beta)
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 235 + 242 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 31, -133 * depth, 126 * depth);
            ...
            else
            {
                ...
                if (history < -4203 * depth)
                    continue;

                history += 69 * thisThread->mainHistory[us][move.from_to()] / 32;

                lmrDepth += history / 3139;

                Value baseFutility = (bestMove ? 47 : 135 + std::abs(history / 277));
                Value futilityValue =
                  ss->staticEval + baseFutility + 108 * lmrDepth + 101 * (ss->staticEval > alpha);
        ...
        if (!rootNode && move == ttData.move && !excludedMove
            && depth >= 7 - (thisThread->completedDepth > 27) + ss->ttPv && is_valid(ttData.value)
            && !is_decisive(ttData.value) && (ttData.bound & BOUND_LOWER)
            && ttData.depth >= depth - 3)
        {
            Value singularBeta  = ttData.value - (59 + 80 * (ss->ttPv && !PvNode)) * depth / 55;
            ...
            {
                int corrValAdj1  = std::abs(correctionValue) / 120694;
                int corrValAdj2  = std::abs(correctionValue) / 128820;
                int doubleMargin = -5 + 241 * PvNode - 195 * !ttCapture - corrValAdj1
                                 - 513 * ttMoveHistory / 65536
                                 - (ss->ply > thisThread->rootDepth) * 48;
                int tripleMargin = 83 + 273 * PvNode - 240 * !ttCapture + 89 * ss->ttPv
                                 - corrValAdj2 - (ss->ply * 252 / 128 > thisThread->rootDepth * 387 / 128) * 54;
        ...
        if (ss->ttPv)
            r -= 2429 + PvNode * 957 + (ttData.value > alpha) * 883
               + (ttData.depth >= depth) * (898 + cutNode * 1130);

        // These reduction adjustments have no proven non-linear scaling

        r += 320;  // Base reduction offset to compensate for other tweaks
        r -= moveCount * 63;
        r -= std::abs(correctionValue) / 14324;

        // Increase reduction for cut nodes
        if (cutNode)
            r += 2800 + 924 * !ttData.move;
        ...
        if (capture)
            ...
            ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
                          + (*contHist[1])[movedPiece][move.to_sq()] + 982 * ss->inCheck - 3244;

        // Decrease/increase reduction for moves with a good/bad history
        r -= ss->statScore * 840 / 8192;
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                update_continuation_histories(ss, movedPiece, move.to_sq(), 1418);
            }
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            r -= 1029 * ttMoveHistory / 8192;

            // Note that if expected reduction is high, we reduce search depth here
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 3499) - (r > 4984 && newDepth > 2), !cutNode);
        }
        ...
        if (value + inc > bestValue)
        {
            ...
            if (value + inc > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 17 && !is_decisive(value))
                        depth -= 2;
    ...
    else if (bestMove)
    {
        ...
        if (!PvNode)
            ttMoveHistory << (bestMove == ttData.move ? 811 : -885);
    }
    ...
    {
        int bonusScale = -312;
        bonusScale += std::min(-(ss - 1)->statScore / 100, 321);
        bonusScale += std::min(76 * depth, 504);
        bonusScale += 182 * ((ss - 1)->moveCount > 8);
        bonusScale += 91 * (ss->cutoffCnt <= 3);
        bonusScale += 146 * (!ss->inCheck && bestValue <= ss->staticEval - 111);
        bonusScale += 127 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 82);

        bonusScale = std::max(bonusScale, 0);

        const int scaledBonus = std::min(168 * depth - 102, 1447) * bonusScale;
    ...
    else if (priorCapture && prevSq != SQ_NONE)
    {
        ...
        thisThread->captureHistory[pos.piece_on(prevSq)][prevSq][type_of(capturedPiece)] << 1076;
    }
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 384;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * 760 / rootDelta + !i * reductionScale * 210 / 512 + 1142;
}
...
void update_all_stats(const Position&      pos,
    ...
    int bonus = std::min(150 * depth - 84, 1465) + 300 * (bestMove == ttMove);
    int malus = std::min(777 * depth - 179, 3494) - 29 * moveCount;

    if (!pos.capture_stage(bestMove))
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * 1053 / 1024);
    ...
    else
    {
        ...
        captureHistory[movedPiece][bestMove.to_sq()][capturedPiece] << bonus * 1212 / 1024;
    }

    // Extra penalty for a quiet early move that was not a TT move in
    // previous ply when it gets refuted.
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -malus * 586 / 1024);
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[movedPiece][move.to_sq()][capturedPiece] << -malus * 1449 / 1024;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    static constexpr std::array<ConthistBonus, 6> conthist_bonuses = {
      {{1, 1110}, {2, 628}, {3, 302}, {4, 517}, {5, 127}, {6, 444}}};
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.from_to()] << bonus * 773 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(),
                                  bonus * (bonus > 3 ? 1017 : 871) / 1024);

    int pIndex = pawn_structure_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()]
      << bonus * (bonus > -9 ? 670 : 417) / 1024;
}
