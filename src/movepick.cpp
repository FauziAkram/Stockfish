...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value += threatenedPieces & from ? (pt == QUEEN && !(to & threatenedByRook)   ? 58205
                                                  : pt == ROOK && !(to & threatenedByMinor) ? 24866
                                                  : !(to & threatenedByPawn)                ? 13212
                                                                                            : 0)
                                               : 0;

            // malus for putting piece en prise
            m.value -= (pt == QUEEN ? bool(to & threatenedByRook) * 47079
                                          + bool(to & threatenedByMinor) * 9945
                          : pt == ROOK ? bool(to & threatenedByMinor) * 29879
                          : pt != PAWN ? bool(to & threatenedByPawn) * 14927
                                       : 0);
        }
...
Move MovePicker::next_move(bool skipQuiets) {
    ...
    auto quiet_threshold = [](Depth d) { return -7161 -4126 * d; };
...
top:
    ...
    case GOOD_QUIET :
        ...
        {
            if ((cur - 1)->value > -7168 || (cur - 1)->value <= quiet_threshold(depth))
                return *(cur - 1);
