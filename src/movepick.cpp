...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value += threatenedPieces & from ? (pt == QUEEN && !(to & threatenedByRook)   ? 51873
                                                  : pt == ROOK && !(to & threatenedByMinor) ? 27386
                                                  : !(to & threatenedByPawn)                ? 14058
                                                                                            : 0)
                                               : 0;

            // malus for putting piece en prise
            m.value -= !(threatenedPieces & from)
                       ? (pt == QUEEN ? bool(to & threatenedByRook) * 50221
                                          + bool(to & threatenedByMinor) * 9907
                          : pt == ROOK ? bool(to & threatenedByMinor) * 25266
                          : pt != PAWN ? bool(to & threatenedByPawn) * 15905
                                       : 0)
                       : 0;
        }
...
Move MovePicker::next_move(bool skipQuiets) {
    ...
    auto quiet_threshold = [](Depth d) { return -6410 -4152 * d; };
...
top:
    ...
    case GOOD_QUIET :
        ...
        {
            if ((cur - 1)->value > -7984 || (cur - 1)->value <= quiet_threshold(depth))
                return *(cur - 1);
