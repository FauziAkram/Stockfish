  ...
  for (auto& m : *this)
      if constexpr (Type == CAPTURES)
          m.value =  (7 * int(PieceValue[pos.piece_on(to_sq(m))])
                   + (*captureHistory)[pos.moved_piece(m)][to_sq(m)][type_of(pos.piece_on(to_sq(m)))]) / 32;
      ...
      else if constexpr (Type == QUIETS)
      {
          ...
          // bonus for checks
          m.value += bool(pos.check_squares(pt) & to) * 8192;

          // bonus for escaping from capture
          m.value += threatenedPieces & from ?
                       (pt == QUEEN && !(to & threatenedByRook)  ? 25000
                      : pt == ROOK  && !(to & threatenedByMinor) ? 12500
                      :                !(to & threatenedByPawn)  ? 7500
                      :                                            0 )
                      :                                            0 ;

          // malus for putting piece en prise
          m.value -= !(threatenedPieces & from) ?
                        (pt == QUEEN ?   bool(to & threatenedByRook)  * 25000
                                       + bool(to & threatenedByMinor) * 5000
                                       + bool(to & threatenedByPawn)  * 10000
                       : pt == ROOK  ?   bool(to & threatenedByMinor) * 12500
                                       + bool(to & threatenedByPawn)  * 5000
                       : pt != PAWN ?    bool(to & threatenedByPawn)  * 7500
                       :                                                0 )
                       :                                                0 ;
      }
