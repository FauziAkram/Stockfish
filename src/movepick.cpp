...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value += threatenedPieces & from ? (pt == QUEEN && !(to & threatenedByRook)   ? 52064
                                                  : pt == ROOK && !(to & threatenedByMinor) ? 25000
                                                  : !(to & threatenedByPawn)                ? 14500
                                                                                            : 0)
                                               : 0;
            ...
            m.value -= !(threatenedPieces & from)
                       ? (pt == QUEEN ? bool(to & threatenedByRook) * 50000
                                          + bool(to & threatenedByMinor) * 10450
                          : pt == ROOK ? bool(to & threatenedByMinor) * 24120
                          : pt != PAWN ? bool(to & threatenedByPawn) * 14830
                                       : 0)
                       : 0;
        }
...
Move MovePicker::next_move(bool skipQuiets) {
    ...
    auto quiet_threshold = [](Depth d) { return -3335 * d; };
