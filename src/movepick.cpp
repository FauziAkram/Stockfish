  ...
  for (auto& m : *this)
      ...
      else if constexpr (Type == QUIETS)
      {
          ...
          m.value += threatenedPieces & from ?
                       (pt == QUEEN && !(to & threatenedByRook)  ? 48809
                      : pt == ROOK  && !(to & threatenedByMinor) ? 24433
                      :                !(to & threatenedByPawn)  ? 14176
                      :                                            0 )
                      :                                            0 ;

          // malus for putting piece en prise
          m.value -= !(threatenedPieces & from) ?
                        (pt == QUEEN ?   bool(to & threatenedByRook)  * 51214
                                       + bool(to & threatenedByMinor) * 10471
                                       + bool(to & threatenedByPawn)  * 20811
                       : pt == ROOK  ?   bool(to & threatenedByMinor) * 24353
                                       + bool(to & threatenedByPawn)  * 9488
                       : pt != PAWN ?    bool(to & threatenedByPawn)  * 14911
                       :                                                0 )
                       :                                                0 ;
      }
...
top:
  ...
  case QUIET_INIT:
      if (!skipQuiets)
      {
          ...
          score<QUIETS>();
          partial_insertion_sort(cur, endMoves, -3004 * depth);
      }
