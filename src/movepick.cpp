...
// MovePicker constructor for ProbCut (Corrected definition)
MovePicker::MovePicker(const Position& p, Move ttm, int th, const CapturePieceToHistory* cph) : // Removed MovePicker:: prefix
    pos(p),
    // mainHistory, lowPlyHistory, continuationHistory, pawnHistory are nullptr implicitly
    captureHistory(cph),
    ttMove(ttm),
    threshold(th),
...
void MovePicker::score() { // Correct placement now
...
Move MovePicker::next_move() { // Correct definition now
...
void MovePicker::skip_quiet_moves() { skipQuiets = true; } // Correct definition
