...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value += threatenedPieces & from ? (pt == QUEEN && !(to & threatenedByRook)   ? 50020
                                                  : pt == ROOK && !(to & threatenedByMinor) ? 27373
                                                  : !(to & threatenedByPawn)                ? 13988
                                                                                            : 0)
                                               : 0;

            // malus for putting piece en prise
            m.value -= !(threatenedPieces & from)
                       ? (pt == QUEEN ? bool(to & threatenedByRook) * 47690
                                          + bool(to & threatenedByMinor) * 9520
                          : pt == ROOK ? bool(to & threatenedByMinor) * 25320
                          : pt != PAWN ? bool(to & threatenedByPawn) * 14006
                                       : 0)
                       : 0;
        }
...
Move MovePicker::next_move(bool skipQuiets) {
    ...
    auto quiet_threshold = [](Depth d) { return -6300 -3827 * d; };
...
top:
    ...
    case GOOD_QUIET :
        ...
        {
            if ((cur - 1)->value > -7836 || (cur - 1)->value <= quiet_threshold(depth))
                return *(cur - 1);
