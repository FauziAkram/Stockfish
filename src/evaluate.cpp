...
namespace {
  ...
  int faz3=1064; TUNE(SetRange(950,1130),faz3);
  int faz4=106, faz1=100, faz2=100; TUNE(SetRange(30, 200), faz1, faz2, faz4);
  int faz5=104, faz6=131; TUNE(SetRange(50, 200), faz5, faz6);
  int faz7=269; TUNE(SetRange(160, 400), faz7);
  int faz8=754; TUNE(SetRange(630, 870), faz8);
  int faz9=195, faz10=211; TUNE(SetRange(150, 260), faz9, faz10);
...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  if (useClassical)
    ...
    v = v * (faz1 - pos.rule50_count()) / faz2;
  }
  else
  {
      int nnueComplexity;
      int scale = faz3 + faz4 * pos.non_pawn_material() / 5120;
      Value optimism = pos.this_thread()->optimism[stm];

      Value nnue = NNUE::evaluate(pos, true, &nnueComplexity);
      // Blend nnue complexity with (semi)classical complexity
      nnueComplexity = (faz5 * nnueComplexity + faz6 * abs(nnue - psq)) / 256;
