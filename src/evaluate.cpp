...
namespace {
  ...
  constexpr int KingAttackWeights[PIECE_TYPE_NB] = { 0, 0, 73, 46, 45, 15 };
...
#undef S
  ...
  Score Evaluation<T>::king() const {
    ...
    kingDanger +=        kingAttackersCount[Them] * kingAttackersWeight[Them] // (~10 Elo)
                 + 184 * popcount(kingRing[Us] & weak)                        // (~15 Elo)
                 + 150 * popcount(unsafeChecks)                               // (~4 Elo)
                 +  97 * popcount(pos.blockers_for_king(Us))                  // (~2 Elo)
                 +  70 * kingAttacksCount[Them]                               // (~0.5 Elo)
                 +  31 * kingFlankAttack * kingFlankAttack / 80                // (~0.5 Elo)
                 +       mg_value(mobility[Them] - mobility[Us])              // (~0.5 Elo)
                 - 870 * !pos.count<QUEEN>(Them)                              // (~24 Elo)
                 - 101 * bool(attackedBy[Us][KNIGHT] & attackedBy[Us][KING])  // (~5 Elo)
                 -  62 * mg_value(score) / 80                                 // (~8 Elo)
