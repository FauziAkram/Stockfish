...
Value Eval::evaluate(const Eval::NNUE::Networks& networks, const Position& pos, int optimism) {
    ...
    optimism += optimism * (nnueComplexity + std::abs(simpleEval - nnue)) / 515;
    nnue -= nnue * (nnueComplexity + std::abs(simpleEval - nnue)) / 32391;

    int npm = pos.non_pawn_material() / 64;
    int v   = (nnue * (917 + npm + 9 * pos.count<PAWN>()) + optimism * (151 + npm)) / 1027;

    // Damp down the evaluation linearly when shuffling
    int shuffling = pos.rule50_count();
    v             = v * (194 - shuffling) / 217;
