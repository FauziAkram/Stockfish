...
namespace Stockfish {
int ww1=915, ww2=154, ww3=220, ww4=2550, ww5=1050;
TUNE(ww1,ww2,ww3,ww4,ww5);
namespace Eval {
...
Value Eval::evaluate(const Position& pos, int optimism) {
    ...
    bool lazy = std::abs(simpleEval) > ww4;
    if (lazy)
        v = simpleEval;
    else
    {
        bool smallNet = std::abs(simpleEval) > ww5;
        ...
        v       = (nnue * (ww1 + npm + 9 * pos.count<PAWN>()) + optimism * (ww2 + npm)) / 1024;
    }

    // Damp down the evaluation linearly when shuffling
    v = v * (200 - shuffling) / ww3;
