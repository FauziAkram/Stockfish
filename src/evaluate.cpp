...
Value Eval::evaluate(const Eval::NNUE::Networks& networks, const Position& pos, int optimism) {
    ...
    const auto adjustEval = [&](int optDiv, int pawnCountConstant, int pawnCountMul,
                                int npmConstant, int evalDiv, int shufflingConstant,
                                int shufflingDiv) {
        // Blend optimism and eval with nnue complexity and material imbalance
        optimism += optimism * (nnueComplexity + std::abs(simpleEval - nnue)) / optDiv;
        nnue -= nnue * (nnueComplexity * 5 / 3) / 32500;
    ...
    if (!smallNet)
        adjustEval(513, 919, 11, 145, 1036, 178, 204);
    else if (psqtOnly)
        adjustEval(517, 908, 7, 155, 1019, 224, 238);
    else
        adjustEval(499, 903, 9, 147, 1067, 208, 211);
