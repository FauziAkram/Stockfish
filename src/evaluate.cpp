...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  bool useClassical = !useNNUE || (pos.count<ALL_PIECES>() > 6 && abs(psq) > 1794);
  ...
  {
      int nnueComplexity;
      int scale = 1009 + 5 * pos.count<PAWN>() + 64 * pos.non_pawn_material() / 4096;
      ...
      v = (nnue * scale + optimism * (scale - 736)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (198 - pos.rule50_count()) / 220;
