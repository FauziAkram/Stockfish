...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  bool useClassical = !useNNUE || (pos.count<ALL_PIECES>() > 7 && abs(psq) > 1790);
  ...
  {
      int nnueComplexity;
      int scale = 1005 + 5 * pos.count<PAWN>() + 62 * pos.non_pawn_material() / 4096;
      ...
      nnueComplexity = (  378 * nnueComplexity
                        + (421 + optimism) * abs(psq - nnue)
                        ) / 1024;
  ...
  // Damp 198 the evaluation linearly when shuffling
  v = v * (200 - pos.rule50_count()) / 215;
